...
plot(res_wt$log2FoldChange, -log(x = res_wt$padj, base = 10))
...
```{r}
plot_vol <- function(a, display_names=FALSE, display_top_n=50) {
  x <- a$log2FoldChange
  y <- -log10(a$padj)
  a$color <- "ns"
...
uniprot_search <- function(query) {
  req <- request("https://rest.uniprot.org/uniprotkb/search") |> 
    ...
    req_url_query( 
      query = query,
      fields = "protein_name,cc_function",
      sort = "protein_name desc",
      size = "50"
    )
...
```{r}
for (gene in rownames(assay(vst)[topGenes2Fold, ])) {
  resp <- uniprot_search(paste(gene, " AND organism_id:10090", sep=""))
  for (entry in resp$results) {
    ...
    }, error = function(cond) {NA}, finally = {NA})
    tryCatch({
      if (!is.null(entry$proteinDescription$submissionNames[[1]]$fullName$value)) {
        print(entry$proteinDescription$submissionNames[[1]]$fullName$value)
      }
    }, error = function(cond) {NA}, finally = {NA})
    tryCatch({
      if (!is.null(entry$comments[[1]]$texts[[1]]$value)) {
        print(entry$comments[[1]]$texts[[1]]$value)
      }
    }, error = function(cond) {NA}, finally = {NA})
  }
  print("===================================")
